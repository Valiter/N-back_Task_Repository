...
def get_data():
    ...
    #Файл будет называться Имя_ДатаРождения, и при повторном прохождении теста одним человеком, данные вписываются в файл этого человека (мне каж так сравнивать удобнее будет) + файл записывается в ту же папку, где лежит программа
    LogFile = Name_student + '_' + Date_student + '.txt'
    ...
    #Results.write(expInfo['1. Испытуемый:']+'\t'+expInfo['2. Дата рождения:']+'\t'+expInfo['4. Дата тестирования:']+ '\n')
    Results.write(Name_student + '\t' + Date_student + '\t' + str(datetime.date(datetime.now())) + '\t' + Dict_student + '\t' + Number_of_elements_student + '\t' + Length_student + '\t' + Interval_student + '\t' + Step_student )
    Results.close()
    window.destroy()
...
Next_btn = Button(window, text = 'Далее', command = get_data) #при нажатии кнопки происходит запись данных в файл и закрытие окна
